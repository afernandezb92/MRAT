...
public abstract class Mensaje {
	...
	protected static final int ONEMINUTE = 360000;
	...
	public static class RGetInfo extends Mensaje {
        ...
        public void decipherMsg(byte[] buffer, int size) throws Exception {
            ...
            System.out.println("info size " + sizeInfo);
            info = unmarshallString(Arrays.copyOfRange(decipherMsg, INTSIZE, sizeInfo + INTSIZE));
            nonce = unmarshallInt(Arrays.copyOfRange(decipherMsg, sizeInfo + INTSIZE, 2*INTSIZE + sizeInfo));
            System.out.println("nonce: " + nonce);
            byte[] tsBytes = new byte[size - (2*INTSIZE + sizeInfo)];
            tsBytes = Arrays.copyOfRange(decipherMsg, 2*INTSIZE + sizeInfo, size);
            System.out.println(new String(tsBytes, "UTF-8"));
            timestamp = Timestamp.valueOf(new String(tsBytes, "UTF-8"));
            if (debug) {
    ...
    public static class RScreenshot extends Mensaje {
        ...
        public void decipherMsg(byte[] buffer, int size) throws Exception {
            ...
            screenshot = Arrays.copyOfRange(decipherMsg, INTSIZE, sizeScreenshot + INTSIZE);
            nonce = unmarshallInt(Arrays.copyOfRange(decipherMsg, INTSIZE + sizeScreenshot, 2*INTSIZE + sizeScreenshot));
        ...
        public void writeTo(OutputStream o) throws Exception {
            ...
            if(debug){
            	System.out.println("Nonce " + nonce);
                System.out.println("Ts " + timestamp);
                System.out.println("info length " + screenshot.length);
                System.out.println(msgCipher.length);
            }           
            output.write(marshallInt(msgCipher.length));
            output.write(msgCipher);
	...
	public static class RErr extends Mensaje {
        ...
        public void decipherMsg(byte[] buffer, int size) throws Exception {
            ...
            tsBytes = Arrays.copyOfRange(decipherMsg, 2*INTSIZE + errorSize, size);
            timestamp = Timestamp.valueOf(new String(tsBytes, "UTF-8"));
            if (debug) {
                System.out.println("nonce: " + nonce);
                System.out.println("ts: " + timestamp);
